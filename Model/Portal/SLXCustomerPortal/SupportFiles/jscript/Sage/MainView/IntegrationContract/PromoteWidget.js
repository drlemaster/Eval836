...
function (
    ...
    var promoteWidget = declare('Sage.MainView.IntegrationContract.PromoteWidget', [_Widget, _Templated], {
        ...
        _requestBackOffices: function() {
            ...
            request.setResourceKind("bodMappings");
            request.setQueryArg("select", "BackOffice/LogicalId,BackOffice/BackOfficeName");
            var where = dString.substitute("EntityName eq '${0}' and IsActive and OutboundOnCreate ne '2' and BackOffice.IsActive", [this.entityName]);
            request.setQueryArg("where", where);
            request.read({
                success: lang.hitch(this, function(result) {
                    if (result) {
                        var count = result.$resources.length;
                        var defaultBackOffice = this.logicalId ? this.logicalId : count === 1 && result.$resources[0].BackOffice.BackOfficeName;
                        var offices = [];
                        dArray.forEach(result.$resources, dojo.hitch(this, function(data) {
                            offices.push(data.BackOffice);
                        }));
                ...
                failure: function(result) {
                    dialogs.showError(dString.substitute(this.errorRequestListOptions, [this.errorBackOffices, result]));
                }
            });
        },
