...
function(topic, declare, _Widget, toolTip, baseSDataStore, util, tooltip, nls, lang, _localStorageMixin, SDataServiceRegistry) {
    var widget = declare('Sage.UI.Controls.PickList', [_Widget, _localStorageMixin], {
        ...
        */
            ...
            this.dataStore.fetch({
                onComplete: lang.hitch(this, function(data) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        if (filter) {
                            pickListData = this.filterPickListItems(pickListData);
                        }
        ...
        filterPickListItems: function (picklist) {
                if (this.filter) {
                    var filteredResults = [];
                    for (var i = 0; i < picklist.items.$resources.length; i++) {
                        var itm = picklist.items.$resources[i];
                        if (itm.filter === this.filter) {
                            filteredResults.push(itm);
                        }
                    }
                    picklist.items.$resources = filteredResults;
                }
                return picklist;
        },
