...
import MODEL_NAMES from 'icboe/Models/Names';
...
const __class = declare('icboe.Promote', [_Widget, _Templated], {
  ...
  processBackOfficeEntries: function processBackOfficeEntries(entries) {
    ...
    this._backOfficeEntries.forEach((entry) => {
      this._backOffices.push({
        value: entry.BackOfficeName,
        key: entry.$key,
      });
    });
    if (this._backOfficeEntries[0]) {
      this.getAccountingEntitiesEntries(this._backOfficeEntries[0].$key);
    } else {
  ...
  processAccountingEntries: function processAccountingEntries(entries) {
    ...
    this._accountingEntities = [];
    this._accountingEntitiesEntries.forEach((entry) => {
      this._accountingEntities.push({
        value: entry.Name,
        key: entry.$key,
      });
    });
    if (this._firstLoad) {
      ...
      if (this._backOffices.length !== 1 || this._accountingEntities.length !== 1) {
  ...
  createAccountingDropdown: function createAccountingDropdown() {
    ...
    this._accountingDropdown.createList({ items: this._accountingEntities, defaultValue: this._accountingEntities[0] ? this._accountingEntities[0].key : '' }); // TODO: change the defaultValue selected
  ...
  createBackOfficeDropdown: function createBackOfficeDropdown() {
    if (!this._backOfficeDropdown) {
      ...
      this._backOfficeDropdown.createList({ items: this._backOffices, defaultValue: this._backOffices[0] ? this._backOffices[0].key : '' }); // TODO: change the defaultValue selected
