...
function (
    ...
    var widget = declare('Sage.UI.Controls.FieldAttributeControlFactory', [GeneralForm], {
        ...
        _createForm: function () {
            ...
            if ((typeof (this.store.StorageOptions) !== "undefined" && this.store.show && (this.store.show.indexOf("Storage") >= 0))) {
                ...
                if (hidden) {
                    ...
                    this.createAttributeStorage('Storage', this._createCustomFilteringSelect(), this.store.Storage, this.store.StorageOptions, this.viewOnly, false);
                }
            }
            if ((typeof (this.store.StorageOptions) !== "undefined" && this.store.show && (this.store.show.indexOf("Display") >= 0))) {
                this.createAttributeStorage('Display', this._createCustomFilteringSelect(), this.store.Display, this.store.DisplayOptions, this.viewOnly, false);
            }
            ...
            if ((typeof (this.store.ValueStoredAsText) !== "undefined" && this.store.show && this.store.show.indexOf("ValueStoredAsText") >= 0)) {
                this.createAttributeEditor('ValueStoredAsText', this._createCustomCheckBox(), this.store.ValueStoredAsText, this.viewOnly);
            }
        ...
        createAttributeStorage: function (attach, Widget, val, options, dis, hidden) {
            ...
            cntrl.set('value', val); // allows the picklist onChange override to be applied to provided data.
            this.domClass.remove(cntrl.domNode, "dijitInline");
            this.activeControls.push({ label: nlsResource[attach], name: attach, control: cntrl, isDom: false, template: "rowTemplate", display: hidden ? "display-name" : "display", Include: true, fieldClassAppendix: "typeattributedetails" });
        },
        _pickListOnChange: function (selfAware, value, item) {
            var newDisplayText = "--- " + nlsResource.newPickList + " ---";
            var formObject = {
            ...
            if (typeof (item) === "undefined" || item === null) {
                ...
                request1.setResourceKind('picklists')
                    ...
                    .setQueryArg('where', dString.substitute("lower(name) eq '${0}'", [value]));
            ...
            if (preExisting) { // populate with data here if we know we have data to use
                ...
                request2.setResourceKind('picklists("' + formObject.$key + '")')
